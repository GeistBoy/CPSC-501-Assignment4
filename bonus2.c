...
double *readWavFile(char *name, int channel)
{
    ...
    if(meta.num_channels == MONOPHONIC){
        for (int i = 0; i <= count; i++)
        {
            int value = ((short int *)buffer)[i];
            input_signal[i] = (double)value / 32768;
...
void writeFile(char *name, double *outputSignalL, double *outputSignalR, int size)
{
    ...
    printf("Now writing the output file ...\n");
    writeWaveFileHeader(chan, size, SAMPLE_RATE, outputStrem);
...
int main(int argc, char *argv[])
{
    ...
    if(!strstr(argv[2], ".wav")){
        // convert files
        char command[100];
        strcpy(command, "sox ");
        strcat(command, argv[2]);
        strcat(command, " ir.wav");
        system(command);
        argv[2] = "ir.wav";
    }
    // Left channel == 0
    // Right channel == 1
    double *irSignalL, *irSignalR;
    irSignalL = readWavFile(argv[2], 0);
    if (chan == 2){
        irSignalR = readWavFile(argv[2], 1);
    }
    ...
    if(!strstr(argv[3], ".wav")){
        writeFile("output.wav", outputSignalL, outputSignalR, outputSize);
        // convert files
        char command[100];
        strcpy(command, "sox output.wav ");
        strcat(command, argv[3]);
        system(command);
        printf("File is converted to %s\n", argv[3]);
    }else{
        writeFile(argv[3], outputSignalL, outputSignalR, outputSize);
    }
